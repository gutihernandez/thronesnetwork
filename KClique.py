...
for characterRow in range(Matrix.__len__()):
    for character in range(Matrix.__len__() - (characterRow+1)):
        if Matrix[characterRow][character + (characterRow + 1)] > 35:
...
for characterRow in range(Matrix.__len__()):
    for character in range(Matrix.__len__() - (characterRow+1)):
        if Matrix[characterRow][character + (characterRow+1)] > 35:
            ...
            source = char_int_to_str[characterRow][0]
            target = char_int_to_str[character + (characterRow + 1)][0]
            value = Matrix[characterRow][character + (characterRow + 1)]
            line = source+ ' ' + target + ' ' + str(value) + '\n'
...
def kCliqueVSsimRank(threshold):
    truePositive = 0
    falsePositive = 0
    for character in char_int_to_str:
        characterToCompare = char_int_to_str[character][0]
        ...
        k3CliqueFrozenSet = list(nx.k_clique_communities(G, 3))
        communityList = [list(x) for x in k3CliqueFrozenSet]

        communityDict = {}
        for distinctCommunity in range(0, len(communityList)):
            characterDict = {}
            for distinctCharacter in range(0, len(communityList[distinctCommunity])):
                characterName = communityList[distinctCommunity][distinctCharacter]
                if communityDict.has_key(characterName) == False:
                    mergeList = []
                    communityDict[characterName] = mergeList
                    communityDict[characterName].append(distinctCommunity)
                else:
                    communityDict[characterName].append(distinctCommunity)

        parts = communityDict
        index = 0
        if parts.has_key(characterToCompare):
            groupNo = parts[characterToCompare]


            # print("-------------------------------------------")
            # print("For character: " + characterToCompare + " most similar characters are below:")
            # for character_tuple in sorted_x:
            #     if parts.has_key(character_tuple[0]):
            #         if character_tuple[0] != characterToCompare:
            #              if index >= sizeFriendsBase and character_tuple[1] > threshold:
            #                 print(str(character_tuple) + " has been found as a result of simrank.")
            #                 hasSameCommunity = False
            #
            #                 groupNoCompare = parts[character_tuple[0]]
            #
            #                 for community in groupNo:
            #                     for compareCommunity in groupNoCompare:
            #                         if community == compareCommunity:
            #                             hasSameCommunity = True
            #
            #                 if hasSameCommunity:
            #                     truePositive += 1
            #                     print("TRUE POSITIVE!!")
...
l1=list(nx.find_cliques(G))
l3= list(nx.k_clique_communities(G, 3))
...
print communityDict
# communityList = [list(x) for x in l3]
print "---"
# print communityList
print "---"
print l3[1]
print l3[0].difference(l3[1])

l4 = list(nx.k_clique_communities(G, 4))
l5 = list(nx.k_clique_communities(G, 5))

kCliqueVSsimRank(1.2)





# l2 = list(get_percolated_cliques(G, 3))
# print x
# l1 =list(get_percolated_cliques(G,7))
print(5)